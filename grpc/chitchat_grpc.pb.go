...
type ChitChatClient interface {
	...
	Join(ctx context.Context, in *JoinRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Broadcast], error)
...
func (c *chitChatClient) Join(ctx context.Context, in *JoinRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Broadcast], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &ChitChat_ServiceDesc.Streams[0], ChitChat_Join_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[JoinRequest, Broadcast]{ClientStream: stream}
...
type ChitChat_JoinClient = grpc.ServerStreamingClient[Broadcast]
...
type ChitChatServer interface {
	// Client joins the chat and receives a stream of broadcasts (chats)
	Join(*JoinRequest, grpc.ServerStreamingServer[Broadcast]) error
	// Client sends a message to be broadcasted to others
...
func (UnimplementedChitChatServer) Join(*JoinRequest, grpc.ServerStreamingServer[Broadcast]) error {
...
func _ChitChat_Join_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(ChitChatServer).Join(m, &grpc.GenericServerStream[JoinRequest, Broadcast]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ChitChat_JoinServer = grpc.ServerStreamingServer[Broadcast]
